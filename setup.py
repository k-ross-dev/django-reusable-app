...
version = get_version("src", "django_reusable_app", "__init__.py")
...
setup(
    ...
    author_email='Your email used for package publication',
    url='https://github.com/Used to initialize GIT repo origin remote/django-reusable-app',
    packages=find_packages('src', exclude=["*.tests", "*.tests.*", "tests.*", "tests"]),
    package_dir={'': 'src'},
    py_modules=[splitext(basename(path))[0] for path in glob('src/*.py')],
    entry_points={
        'console_scripts': [
            'django_reusable_app=django_reusable_app.cli:main'
        ]
    },
